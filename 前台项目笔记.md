...
```
  searchForTrademark(trademark){
      ...
      // 示例: "1:苹果"
      // 'id:name'
      this.searchParams.trademark = `${trademark.tmId}:${trademark.tmName}`  // 这里根据品牌搜索一定要参考文档去看参数的结构
...
* render: "TypeError: Cannot read property 'split' of undefined"
...
```
 <ul class="fl sui-tag">
            ...
            <!-- 
              split  字符串方法 以某一个子串为切割点 切割成数组
              v-show 和 v-if 的区别在哪?
                v-show:是用display:none;把这个东西隐藏掉了,但是他还是会渲染
                但是刚开始里边没有trademark 里边是空串  
                空串被过滤掉了,
                v-show : 只是在判断 searchParams.trademark 存不存在
                        如果它不存在,相当于写了个display:none;也就是用css隐藏掉了,但是它下边的
                        {{searchParams.trademark.split(':')[1]}} 还是要渲染的
                        渲染的时候 就去   {{searchParams.trademark.split(':')[1]}}  这个里边拿 searchParams里边的trademark
                        拿到之后再去split  但是split拿到的是undefined，undefined.会报错的
                  ...
                  一上来是空串被过滤掉了   
                  没点  v-show要判断 
                  searchParams.trademark 是空的 v-show拿到了undefined  undefined是false
                  false  它内部所有的东西仍然要渲染,但是它用的是display:none; 用css给隐藏掉了  仅此而已
                  而它下边要渲染的时候,又进一步的 .split === undefined.split  报错

                  而v-if  如果条件中的内容不存在 就为false  下边的内容不渲染  
...
```
 <li v-for="(attrValue, index) in attr.attrValueList" :key="index">
            ...
            <!-- 属性ID(attr):属性值(attrValue):属性名(attr) -->
          </li>
```
* 2.Search ---> index.vue
```
...
```
    ...
    searchForProps(){
      ...
      // every()  : 数组当中每一个都符合条件才为true  
      // 加入之前对判断数据当中是否存在,如果已经存在就不需要再次加入并且发送请求
      let repeat = this.searchParams.props.some(item => item === prop)  //只要有一个符合条件,整体返回的就是true
      if(repeat) return
...
```
1.
<li class="with-x" v-for="(item, index) in searchParams.props" :key="index">{{prop.split(':')[1]}}<i @click="removeProp(index)">×</i></li>
